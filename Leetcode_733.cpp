...
public:
    ...
    vector<vector<int>> floodFill(vector<vector<int>>& image, int sr, int sc, int newColor) {
        ...
        loopNode(image[sr][sc], sr, sc, image, newColor);
        return image;
    }

    void loopNode(int eql, int row, int col, vector<vector<int>>& image, int newColor){
        ...
        for(int i=0;i <4;i++){
            ...
            if(x >=0 && x < rowSize && y >=0 && y < colSize)
                loopNode(eql, x, y, image, newColor);
        }
    }
};
